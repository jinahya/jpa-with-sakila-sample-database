...
import static com.github.jinahya.persistence.sakila.__Activatable.ATTRIBUTE_NAME_ACTIVE;
import static com.github.jinahya.persistence.sakila.util.InvokeUtils.findVarHandle;
import static com.github.jinahya.persistence.sakila.util.ReflectionUtils.findFieldNamed;
...
class __ActivatableHelper {
    ...
    static Field getActiveField(final Class<?> cls) {
        ...
        return ACTIVE_FIELDS.computeIfAbsent(
                ...
                k -> {
                    try {
                        return findFieldNamed(Class.forName(k), ATTRIBUTE_NAME_ACTIVE, Integer.class);
    ...
    static VarHandle getActiveVarHandle(final Class<?> cls) {
        ...
        return ACTIVE_VAR_HANDLES.computeIfAbsent(
                ...
                k -> {
                    try {
                        return findVarHandle(Class.forName(k), ATTRIBUTE_NAME_ACTIVE, Integer.class);
                    } catch (final ReflectiveOperationException roe) {
                        throw new RuntimeException(roe);
                    }
                }
