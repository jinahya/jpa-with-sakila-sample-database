...
import com.github.jinahya.sakila.persistence.service.ActorService;
 ...
 */
@NamedEntityGraph(
        name = ActorConstants.ENTITY_GRAPH_FILMS,
        attributeNodes = {
                @NamedAttributeNode(ActorConstants.ATTRIBUTE_NODE_FILMS)
        }
)
@NamedQuery(
        name = ActorConstants.QUERY_FIND_ALL_BY_LAST_NAME,
        query = """
                SELECT e
                FROM Actor AS e
...
@NamedQuery(
        name = ActorConstants.QUERY_FIND_ALL,
        query = """
                SELECT e
                FROM Actor AS e
...
@NamedQuery(
        name = ActorConstants.QUERY_FIND_BY_ACTOR_ID,
        query = """
                SELECT e
                FROM Actor AS e
...
@NamedNativeQuery(
        name = ActorConstants.NATIVE_QUERY_SELECT_ALL,
        query = """
                SELECT *
                FROM actor
...
@NamedNativeQuery(
        name = ActorConstants.NATIVE_QUERY_SELECT_BY_ACTOR_ID,
        query = """
                SELECT *
                FROM actor
                WHERE actor_id = ?""",
